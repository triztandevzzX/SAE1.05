...
try:
    with open("/Users/tristan/Documents/1ere_année/SAE/SAE1.5/DumpFile.txt", encoding="utf8") as fh:
        res=fh.read()
except:
...
fic=open("tableau.csv", "w")#tableau est le fichier .csv avec les valeurs
evenement = "DATE ; SOURCE ; PORT ; DESTINATION ; FLAG ; SEQ ; ACK ; WIN ; OPTIONS ; LENGTH" #nom des colonnes
fic.write(evenement + "\n") #écriture des valeurs dans le tableau
...
for event in ress:
        if event.startswith('11:42'): #évenement qui commence par "11:42" (ils commencent tous par 11:42)
            ...
            # Flag
            texte=event.split("[") #On coupe la ligne à partir du crochet
